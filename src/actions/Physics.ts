...
*/
function calculateDistanceBetweenTwoPoints (coords1: [number, number], coords2: [number, number]) {
  return Math.sqrt(Math.pow(Math.abs(coords1[0]/2-coords2[0]),2) + Math.pow(Math.abs(coords2[1]/2-coords2[1]),2))
}

/*
  https://happycoding.io/tutorials/processing/collision-detection
*/
type DoCirclesCollideArgs = [Parameters<typeof calculateDistanceBetweenTwoPoints>[0],
...
export function calculatePhysics(GameWorldSingleton) {
  /*
    GameWorldSingleton
...
*/
const [ship, asteroid] = GameWorldSingleton.getInstance.entities() as [Ship, Entity];  
  if(doCirclesCollide)
  return
}