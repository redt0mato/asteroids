...
var _entity = require("./objects/Entity");
var _controller = require("./actions/Controller");
...
class GameWorld {
    constructor(ctx){
        ...
        const shipInstance = new (0, _entity.Ship)(ctx, width / 2, height / 2, 0, 0);
        this.ctx = ctx;
        GameWorld._instance.entities = [
            shipInstance
        ];
        for(let i = 0; i < 5; i++)GameWorld._instance.entities.push(new (0, _entity.Asteroid)(ctx, Math.random() * canvas.width, Math.random() * canvas.height));
        (0, _controller.setupControls)(shipInstance, GameWorld);
    ...
    requestGameAnimationFrame() {
        ...
        (0, _physics.calculatePhysics)(GameWorld);
...
parcelHelpers.export(exports, "EntityTypes", ()=>EntityTypes);
...
parcelHelpers.export(exports, "Asteroid", ()=>Asteroid);
...
class Ship extends Entity {
    ...
    fireBullet(GameWorld) {
        ...
        const newBullet = new Bullet(ctx, shipXPos + 2.25 * radius * (shipVelocity[0] / Math.abs(shipVelocity[0] || 1)), shipYPos + 2.25 * radius * (shipVelocity[1] / Math.abs(shipVelocity[1] || 1)), shipVelocity[0] / Math.abs(shipVelocity[0] || 1) * 3, shipVelocity[1] / Math.abs(shipVelocity[1] || 1) * 3);
        const a = new Set([
            "hi"
        ]);
...
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "setupControls", ()=>setupControls);
var _entity = require("../objects/Entity");
function setupControls(shipInstance, GameWorld) {
    document.onkeydown = function(event) {
        if (event.keyCode === 37) shipInstance.power((0, _entity.DIRECTIONS).LEFT);
        else if (event.keyCode === 38) shipInstance.power((0, _entity.DIRECTIONS).UP);
        else if (event.keyCode === 39) shipInstance.power((0, _entity.DIRECTIONS).RIGHT);
        else if (event.keyCode === 40) shipInstance.power((0, _entity.DIRECTIONS).DOWN);
        else if (event.keyCode === 32) shipInstance.fireBullet(GameWorld);
    };
}

},{"../objects/Entity":"gHrh4","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"1UKZe":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "calculatePhysics", ()=>calculatePhysics);
...
function calculatePhysics(GameWorld) {
    ...
    let entity1, entity2;
    for(let i = 0; i < entitiesArray.length; i++)for(let j = i + 1; j < entitiesArray.length; j++){
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["37eO2","7SwCM"], "7SwCM", "parcelRequire94c2")
